...
exports.methods = {
    ...
    async setTalkingHeadId(userId) {
        const sessionId = this.$.session.getSessionId();
        if (sessionId === APP_SESSION) {
            // only the main client controls who is talking
            this.state.userId = userId;
            notifyWebApp(this, `UserId updated with ${userId}`);
        } else {
            this.$.log.debug(`Ignoring userId ${userId} from ${sessionId}`);
        }
        return this.getState();
    },
    ...
    async setBlur(isBlur) {
        this.state.blur = isBlur;
        notifyWebApp(this, `Set blur to ${isBlur}`);
        return this.getState();
    },
    ...
    async receiveToken(app, token) {
        ...
        notifyWebApp(this, `Updating WAB with ${app}`);
        return this.getState();
    },
